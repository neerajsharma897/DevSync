      ...
      <div className="px-8 pt-8 pb-4 border-b border-gray-800/60 shrink-0">
        ...
        <div className="flex space-x-6">
          <button 
            ...
            className={`pb-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'commits' ? 'border-purple-500 text-purple-400' : 'border-transparent text-gray-400 hover:text-gray-200'}`}
          >
            Recent Commits
          </button>
          <button 
            onClick={() => setActiveTab('ci')}
            className={`pb-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'ci' ? 'border-purple-500 text-purple-400' : 'border-transparent text-gray-400 hover:text-gray-200'}`}
          >
            CI/CD Pipelines
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-8">
        {activeTab === 'commits' ? (
          <div className="space-y-4">
            ...
            <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-4 flex items-start space-x-4">
              ...
              <div className="flex-1">
                <div className="flex items-center space-x-2 mb-1">
                  ...
                  <span className="bg-purple-500/10 text-purple-400 border border-purple-500/20 text-xs px-1.5 rounded">{key}-42</span>
                </div>
            ...
            {/* Fake PR */}
            <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-4 flex items-start space-x-4">
              <div className="mt-1 bg-emerald-500/10 p-1.5 rounded text-emerald-500 border border-emerald-500/20">
                <GitPullRequest className="w-4 h-4" />
              </div>
              <div className="flex-1">
                <div className="flex items-center space-x-2 mb-1">
                  <span className="text-sm font-bold text-gray-200">Fix authentication middleware layout</span>
                  <span className="bg-purple-500/10 text-purple-400 border border-purple-500/20 text-xs px-1.5 rounded">{key}-12</span>
                </div>
                <div className="text-xs text-gray-500">
                  <span className="text-emerald-400">#45 merged</span> into main by aliceweb
                </div>
              </div>
            </div>
          </div>
