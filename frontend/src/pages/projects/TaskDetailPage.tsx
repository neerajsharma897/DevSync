        ...
        <div className="col-span-8 space-y-8">
          ...
          <div>
            ...
            <div className="flex items-center space-x-4">
              <select
                ...
                onChange={async (e) => {
                  try {
                    await apiFetch(`/workspaces/${slug}/projects/${key}/tasks/${taskKey}/status`, {
                      method: 'PATCH',
                ...
                className="flex items-center space-x-2 px-3 py-1.5 bg-gray-900 border border-gray-700 text-gray-300 rounded hover:bg-gray-800 transition-colors text-sm font-semibold uppercase tracking-wide focus:outline-none"
              >
          ...
          <div>
            ...
            <div className="flex space-x-4 mb-6">
              <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-white text-xs font-bold shrink-0">ME</div>
              <div className="flex-1">
                <input 
                  type="text" 
                  placeholder="Add a comment..." 
                  className="w-full bg-gray-900 border border-gray-800 rounded-lg px-4 py-2.5 text-sm text-gray-200 focus:outline-none focus:border-white/50 focus:ring-1 focus:ring-white/50"
                />
              </div>
            </div>
          ...
          <div className="bg-gray-900/40 border border-gray-800/80 rounded-xl p-5 space-y-5">
            ...
            <div className="grid grid-cols-3 gap-y-4 text-sm">
              ...
              <div className="col-span-2 flex items-center space-x-2 text-gray-200">
                {task.assigneeId ? (
                  <><div className="w-5 h-5 rounded-full bg-white flex items-center justify-center text-[10px] text-white">U</div> <span>User Assigned</span></>
                ) : (
              ...
              <div className="col-span-2 flex flex-wrap gap-1">
                {task.labels?.map((label, idx) => (
                  <span key={idx} className="bg-white/10 border border-white/20 text-gray-300 text-xs px-2 py-0.5 rounded">
                    {label}
                  </span>
                )) || '-'}
              </div>
